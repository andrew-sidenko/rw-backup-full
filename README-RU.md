# rw-backup-full v5.5

Полнофункциональная система резервного копирования панелей Remnawave и кастомных Telegram-ботов: логические бэкапы (формат полностью совместим с [distillium/remnawave-backup-restore](https://github.com/distillium/remnawave-backup-restore)), непрерывная WAL-архивация PostgreSQL с PITR, несколько внешних S3 с индивидуальными сроками хранения, автоматическая проверка бэкапов в песочнице, веб-сервис управления парком серверов и дашборд Grafana.

**v5: весь функционал оригинального rw-backup встроен** — панельные бэкапы и восстановление выполняются собственным движком из единого меню и единого конфига; оригинальный скрипт больше не требуется (но архивы взаимно совместимы в обе стороны).

**v5.5:** меню зависит от `FULL_COMPONENTS` (на песочнице не показываются чужие разделы); S3/TG реквизиты копируются с серверов парка при проверках; сводки в Telegram в 09:00 и 21:00; журналы `.txt` рядом с архивами в S3. Подробнее: [docs/FLEET-VERIFY-RU.md](docs/FLEET-VERIFY-RU.md), [CHANGELOG.md](CHANGELOG.md).

**v5.6:** тяжёлые операции (логический дамп + базовый бэкап) по умолчанию идут ночью и разнесены по времени, WAL — непрерывно; случайный сдвиг `FULL_SCHEDULE_JITTER_SEC`, чтобы серверы не били в одно S3 одновременно. Строгий режим выгрузки `FULL_S3_STRICT` (алерт, если копия ушла не во все хранилища). Сводки 09:00/21:00 показывают объём, занятый в каждом S3-хранилище. Веб-панель по каждому серверу показывает объёмы (локально и в S3), состояние хранилищ, свежесть базы/WAL, ошибки и вердикт последней проверки, плюс сводную карточку «Парк».

**v5.6.13:** отдельный автономный проект [`rw-backup-verify/`](rw-backup-verify/README-RU.md) —
проверка логических бэкапов из S3 на отдельном сервере без веб-парка и SSH.

## Что нового в v4

| Возможность | v3 | v5 |
|---|---|---|
| Логический бэкап панели (через оригинальный rw-backup) | ✅ | ✅ |
| Бэкап кастомных ботов из `/home` (pg_dumpall + Redis + каталог) | ✅ | ✅ |
| Дублирование во внешний S3, age-шифрование, Telegram | ✅ | ✅ |
| **Непрерывная WAL-архивация PostgreSQL (локально + S3)** | — | ✅ |
| **Полные базовые бэкапы `pg_basebackup` по расписанию** | — | ✅ |
| **Восстановление из полного бэкапа и/или WAL (PITR)** | — | ✅ |
| **Автопроверка бэкапов в песочнице на отдельном сервере** | — | ✅ |
| **Метрики в VictoriaMetrics (textfile collector)** | — | ✅ |
| **Встроенный бэкап/восстановление панели (без оригинального скрипта)** | — | ✅ |
| **Несколько внешних S3, у каждого свои настройки и retention** | — | ✅ |
| **Веб-сервис контроля/управления парком серверов (на песочнице)** | — | ✅ |
| **Экспортер данных о бэкапах + готовый дашборд Grafana** | — | ✅ |

Ключевая идея v4: логические дампы (`pg_dumpall`) каждые N часов — это потеря до N часов данных при аварии. WAL-архивация снижает потерю до `INST_ARCHIVE_TIMEOUT` секунд (по умолчанию 5 минут) и при этом **разгружает серверы**: тяжёлый `pg_dumpall` можно гонять реже, а непрерывная архивация почти ничего не стоит — это копирование готовых 16-МБ файлов.

## Архитектура WAL-слоя

```
контейнер postgres                 хост                              S3
┌─────────────────────┐   ┌──────────────────────────┐   ┌─────────────────────┐
│ archive_command ────┼──▶│ /var/lib/rw-wal/<inst>/  │   │ <prefix>/wal/<host>/│
│  (атомарный cp,     │   │   spool/incoming/        │   │   <inst>/           │
│   БЕЗ сети)         │   │        │ wal-ship.sh     │   │     basebackup/     │
│                     │   │        ▼ (таймер, 1 мин) │──▶│     wal/            │
│ pg_basebackup ──────┼──▶│   archive/  (сжатие,     │   │                     │
│  (таймер, 24 ч)     │   │   basebackup/  шифрование│   │                     │
└─────────────────────┘   └──────────────────────────┘   └─────────────────────┘
```

Принципиальные решения:

- **`archive_command` не ходит в сеть.** Он только атомарно копирует сегмент в спул на хосте. Если бы он писал в S3 напрямую, любой сбой S3 привёл бы к росту `pg_wal` и остановке БД с заполненным диском. Отправкой занимается отдельный шиппер на хосте — с ретраями, не блокируя PostgreSQL.
- **Образы postgres не модифицируются.** В контейнер монтируется только каталог спула (`docker-compose.override.yml`), параметры ставятся через `ALTER SYSTEM`. Обновления панели ничего не ломают.
- **WAL никогда не удаляется по возрасту.** Retention удаляет сегменты только старше стартового сегмента самого старого хранимого базового бэкапа — отдельно для локального хранилища и для S3. Любой хранимый базовый бэкап всегда восстановим.
- **Локальная копия + S3.** Сегмент удаляется из спула только после попадания в локальный архив; из локального архива — только по границе retention. Отказ S3 не приводит к потере WAL.

## Установка

### Прод-сервер (панель и/или боты)

```bash
git clone https://github.com/andrew-sidenko/rw-backup-full.git
cd rw-backup-full
sudo ./install.sh
```

Зависимости: `docker` + `docker compose`, `flock` (util-linux). Рекомендуется: `awscli` (S3), `zstd` (сжатие быстрее и компактнее gzip), `age` (если нужно шифрование).

```bash
apt update && apt install -y awscli zstd age
```

Установка **идемпотентна и интерактивна**: инсталлятор показывает, что обнаружено (v3/оригинал), выводит план и запрашивает подтверждение на каждый класс изменений с описанием последствий — копирование файлов, миграции настроек, юниты, включение таймеров. Ваши конфиги (`config.env`, `rw-backup-full.env`, `instances.d/`, `s3.d/`) никогда не перезаписываются; каждый заменяемый файл предварительно копируется в `/opt/rw-backup-restore/.install-backup/<дата>/`; существующий таймер v3 и его расписание сохраняются. Для автоматизации массовой раскатки: `sudo ./install.sh --yes`. Тот же принцип информированного согласия действует в опасных операциях самого инструмента: `wal-enable` спрашивает разрешение на рестарт контейнера БД, `panel-restore` — на каждый из 5 шагов.

### Сервер-песочница (проверка бэкапов)

Отдельный недорогой сервер. Нужны только docker и awscli; доступа к проду не требуется — проверяется содержимое S3.

```bash
git clone https://github.com/andrew-sidenko/rw-backup-full.git
cd rw-backup-full
sudo ./install.sh --sandbox
```

## Настройка (прод)

### 1. Базовая настройка v3-части

```bash
sudo rw-backup-full configure-s3        # внешний S3 (бакет, ключи, endpoint)
sudo rw-backup-full configure-telegram  # уведомления
sudo rw-backup-full install-timer       # периодичность логических бэкапов
```

Все настройки живут в `/opt/rw-backup-restore/rw-backup-full.env`. Оригинальный `config.env` distillium-скрипта не изменяется никогда.

### 2. Описание инстансов WAL

Инстанс — одна PostgreSQL-база (панель или бот). На каждый — файл в `/opt/rw-backup-restore/instances.d/<имя>.env`.
Имя файла (= имя инстанса) для панели **всегда `panel`**: от него зависят пути в S3
(`…/config/<host>/panel`, `…/wal/<host>/panel`). `INST_KIND` — тип профиля (`panel`/`bot`/`site`),
не ярлык для веба. Уникальность в общей панели парка — подпись **`<id карточки>-<инстанс>`**
(например карточка `tyler` + инстанс `panel` → `tyler-panel`), а не переименование файла.

```bash
cp /opt/rw-backup-restore/config-examples/instances.d/panel.env.example \
   /opt/rw-backup-restore/instances.d/panel.env
nano /opt/rw-backup-restore/instances.d/panel.env
```

Минимум, который нужно проверить:

```bash
INST_CONTAINER="remnawave-db"                      # имя контейнера БД
INST_COMPOSE_FILE="/opt/remnawave/docker-compose.yml"
INST_COMPOSE_SERVICE="remnawave-db"                # имя сервиса в compose
INST_PGUSER="postgres"                             # как DB_USER в config.env
```

**Периодичность задаёте вы** — для каждого таймера либо интервал, либо список конкретных времён (количество не ограничено; список приоритетнее интервала):

```bash
# Вариант А: интервалы
INST_BASEBACKUP_INTERVAL_HOURS="24"   # полный базовый бэкап каждые 24 ч
INST_WAL_SHIP_INTERVAL_MIN="1"        # отправка WAL каждую минуту

# Вариант Б: конкретные времена (HH:MM или HH:MM:SS, через пробел/запятую)
INST_BASEBACKUP_TIMES="03:00 12:30 21:00"
INST_WAL_SHIP_TIMES=""                # для WAL обычно оставляют интервал (см. ниже)
INST_SCHEDULE_TZ="UTC"                # зона для времён; пусто = локальная сервера

INST_ARCHIVE_TIMEOUT="300"            # макс. возраст незакрытого сегмента, сек (= ваш RPO)
```

Расписания реализованы штатными средствами systemd (`OnCalendar` — по одному правилу на каждое время, `Persistent=true` добирает пропущенные запуски после простоя). После изменения примените: `rw-backup-full wal-timers <инстанс>`.

**По умолчанию тяжёлые операции идут ночью, чтобы не нагружать боевой сервер днём:** логический дамп — `03:00` (`FULL_TIMER_TIMES` в `rw-backup-full.env`), базовый бэкап панели — `04:30`, базовый бэкап бота — `05:00` (сдвиг +30 мин, чтобы не пересекаться). WAL при этом шлётся непрерывно (каждую минуту) — на него ночной режим не влияет. Чтобы серверы парка не начинали выгрузку в одно S3 одновременно, действует случайный сдвиг `FULL_SCHEDULE_JITTER_SEC` (по умолчанию 900 c): в systemd — как `RandomizedDelaySec` таймеров, в контейнере/cron — как пауза в начале тяжёлой операции (WAL-шиппер джиттеру не подвержен).

⚠️ Про `INST_WAL_SHIP_TIMES`: отправка WAL по редкому расписанию увеличивает RPO по S3-копии — сегменты лежат в локальном спуле до следующего запуска шиппера. Задавать времена здесь имеет смысл только при дорогом/лимитированном канале в S3.

**Хранение** (в штуках базовых бэкапов; WAL подчищается автоматически):

```bash
INST_LOCAL_BASEBACKUP_KEEP="2"
INST_S3_BASEBACKUP_KEEP="7"
```

Для ботов — второй пример: `custom-bot.env.example` (контейнер вида `vpn_postgres`, compose в `/home/<проект>/`).

### 3. Включение архивации

⚠️ **Требуется рестарт контейнера БД** (параметр `archive_mode` в PostgreSQL применяется только при старте). Простой — секунды, но выбирайте время.

```bash
sudo rw-backup-full wal-enable panel
```

Скрипт: подготовит спул → создаст `docker-compose.override.yml` с монтированием → выставит `archive_mode/archive_command/archive_timeout` через `ALTER SYSTEM` → пересоздаст контейнер → **сквозным тестом** переключит WAL-сегмент и убедится, что он дошёл до спула → включит таймеры с вашими интервалами.

Затем первый базовый бэкап:

```bash
sudo rw-backup-full basebackup panel
```

С этого момента точка восстановления непрерывно двигается вперёд.

### 4. Проверка состояния

```bash
sudo rw-backup-full wal-status
```

Показывает по каждому инстансу: `archive_mode`, счётчик ошибок архиватора (`pg_stat_archiver.failed_count` — должен быть стабилен), размер спула (рост = шиппер не справляется), количество базовых бэкапов, активность таймеров.

Логи: `journalctl -u rw-wal-ship@panel.service -f`, `journalctl -u rw-basebackup@panel.service`.

## Восстановление

Все сценарии — через `pitr-restore.sh`. **По умолчанию восстановление безопасно**: рабочая БД не трогается, копия поднимается во временном контейнере на свободном порту.

### Посмотреть, что есть

```bash
ls /var/lib/rw-wal/panel/basebackup/*.meta
aws s3 ls s3://BUCKET/rw-backup-full/wal/HOST/panel/basebackup/
```

### Восстановление на последнюю точку (весь доступный WAL)

```bash
sudo rw-backup-full pitr-restore panel --target latest
# => БД доступна на 127.0.0.1:<порт>, рабочая панель не затронута
```

### Восстановление на момент времени (PITR)

Например, за 5 минут до ошибочного `DELETE`:

```bash
sudo rw-backup-full pitr-restore panel --target-time "2026-07-22 09:30:00+00"
```

Дальше два пути: выгрузить нужные данные `pg_dump`-ом из временного контейнера и залить в прод, либо заменить прод целиком (ниже).

### Восстановление из S3 (сервер утрачен)

На новом сервере: установить проект, заполнить `FULL_EXTERNAL_S3_*`, скопировать `instances.d/*.env`, затем:

```bash
sudo rw-backup-full pitr-restore panel --from s3 --target latest
```

### Только полный бэкап, без WAL

```bash
sudo rw-backup-full pitr-restore panel --target immediate --backup base_2026-07-22_03_00_01_0000000100000000000000A7
```

### Замена рабочей БД (деструктивно)

```bash
sudo rw-backup-full pitr-restore panel --target latest --in-place
```

Остановит сервис БД, отложит старый PGDATA в `*.before_restore_<дата>` (не удаляет!), развернёт восстановленные данные и подскажет команды запуска. После успешного старта заново включите архивацию: `wal-enable` (восстановленная БД стартует с чистым `archive_mode`).

### Зашифрованные бэкапы

```bash
sudo rw-backup-full pitr-restore panel --target latest --age-identity /root/age-restore.key
```

Приватный ключ age на прод-сервере не хранится — держите его в менеджере секретов и на песочнице.

### Логические бэкапы (v3)

Работают как раньше: панель — `rw-backup restore` (оригинальный distillium), боты — `rw-backup-full custom-restore`. WAL-слой их дополняет, а не заменяет: логический дамп — это переносимый и понятный формат, PITR — это минимальная потеря данных.

## Песочница: автоматическая проверка бэкапов

Бэкап, который ни разу не восстанавливали, — это лотерея. Песочница ежедневно проверяет **реальную восстановимость** того, что лежит в S3:

1. **PITR-цепочки** каждого инстанса: скачивает свежий базовый бэкап (с проверкой SHA256), проигрывает весь WAL, поднимает временный postgres и выполняет:
   - `SELECT 1` — БД жива и вышла из recovery;
   - `count(*)` по ключевым таблицам из `INST_VERIFY_TABLES` (пустая таблица = провал);
   - полный `pg_dumpall` восстановленной базы — ловит битые страницы, невидимые через count;
   - возраст базового бэкапа (свежесть цепочки).
2. **Логические архивы**: свежие `remnawave_backup_*.tar.gz` и `custom_bot_*.tar.gz` из S3 распаковываются, дамп заливается в чистый временный postgres, проверяется наличие пользовательских таблиц и строк.

Результат: Telegram-отчёт (при провале — всегда) + метрики. Временные контейнеры и данные удаляются автоматически.

### Настройка песочницы

На песочнице с парком (`FULL_COMPONENTS` включает `sandbox` + `web`) **не копируйте**
`instances.d/*.env` с продов в одну общую папку. У каждого боевого сервера свой
`/opt/rw-backup-restore/instances.d/panel.env` с каноническим именем `panel` —
конфликта нет, это разные хосты. Песочница читает описания инстансов по SSH
(`fleet-manifest` → `web-data/manifest-cache.json`), а S3/TG кэширует в
`fleet-creds/<id карточки>/`. Локальный `instances.d/` на песочнице для
проверки парка / полного стека **не используется**.

```bash
sudo ./install.sh --sandbox          # поднимет web + sandbox + metrics
# добавьте серверы в веб-интерфейсе (SSH-ключ сервиса → authorized_keys на хостах)
# SANDBOX_AGE_IDENTITY — если бэкапы шифруются age
sudo rw-backup-full sync-creds       # или пункт меню «Обновить S3/TG креды»
sudo rw-backup-full verify           # → verify-fleet по манифесту парка
```

Расписание — `rw-sandbox-verify.timer`: по умолчанию ежедневно ~05:30; задайте свои времена (`SANDBOX_VERIFY_TIMES="05:30 17:30"`) или интервал (`SANDBOX_VERIFY_INTERVAL_HOURS`) в конфиге и примените `rw-backup-full sandbox-timer`.

Legacy (без `fleet.json`, одна песочница на один бакет) — тогда да, копировали
`instances.d` и S3 на чтение локально; для современного парка это не нужно и
провоцирует переименования вроде `tyler-panel.env`. Подробно: **docs/FLEET-VERIFY-RU.md**.

Рекомендация по правам S3: прод — ключ с записью; на песочнице реквизиты
подтягиваются с серверов в `fleet-creds/` (лучше отдельный read-only ключ на проде).

## Трекер каталогов и полное восстановление (v5.3)

WAL закрывает базу непрерывно, трекер — всё остальное: каталог панели и каталоги проектов ботов **целиком** (конфиги, `.env`, сертификаты, исполняемый код, ресурсы). Модель та же, что у WAL: git-история на хосте, полный bundle в S3 = базовый бэкап, приращения = сегменты; RPO по файлам — минуты. Каталоги данных СУБД исключены, они покрыты WAL.

`verify-stack` отвечает на главный вопрос — поднимется ли проект с нуля из одних бэкапов: каталог из трекера + база из бэкапа + весь стек контейнеров, **в сетевой изоляции** (`--internal`, без published ports, без docker.sock, без внешних сетей и томов). Это принципиально: поднятая копия панели с настоящими секретами иначе перенастроила бы боевые ноды и разослала сообщения реальным пользователям. Подробно: **docs/CONFIG-TRACK-RU.md**.

Зависимость от оригинального distillium-скрипта убрана, вся конфигурация — в одном файле, состав задач сервера задаётся списком `FULL_COMPONENTS`.

## Fleet-verify: сервер × хранилище (v5.2)

Проверка бэкапов выполняется для **каждого подключенного сервера и каждого его S3-хранилища отдельно** — матрицей. Все параметры (реквизиты хранилищ, инстансы, Telegram каждого сервера) песочница извлекает из веб-сервиса автоматически: добавили сервер в веб-интерфейсе — его бэкапы проверяются. Проверяемые параметры зависят от типа БД (`verify-profiles.d/{panel,bot,site}.env`: обязательные таблицы, минимумы, произвольные SQL). Глубина, параллелизм, история архивов и расписание — в `fleet.json` (сервер посвящён песочнице, ресурсы можно утилизировать: `depth=deep` включает pg_dumpall+pg_amcheck и проверку N последних архивов). Всё состояние сервера песочницы переносится одним компактным файлом: `rw-backup-full fleet-pack pack`. Подробно: **docs/FLEET-VERIFY-RU.md**.

## Контейнерный режим (v5.1) и путь в облако

Toolkit собирается в единый образ (`container/Dockerfile`): агент прод-хоста, песочница, веб-сервис и CLI — один версионируемый артефакт вместо раскатки скриптов. Обновление хоста = `docker pull` + `up -d`. Расписания внутри контейнера генерируются из тех же конфигов (`*_TIMES`/интервалы поддержаны), настройки не дублируются. Перевод существующего хоста — `scripts/host/migrate-to-container.sh` (пошаговые подтверждения, откат `--revert`); systemd-вариант остаётся полностью поддержанным.

Схема данных в S3 обобщена до `<prefix>/<категория>/<источник>/` (`RW_SOURCE_ID`, по умолчанию — hostname, полная обратная совместимость). Песочница проверяет каждый источник, метрики и дашборд различают `host`/`k8s` — поэтому при постепенном переезде в k8s (CronJob'ы того же образа, CloudNativePG для БД в кластере) старая и новая инфраструктура видны и проверяются одинаково. Подробно: **docs/CONTAINER-RU.md**. Сами k8s-манифесты намеренно отложены до переезда.

## Несколько внешних S3

Хранилищ может быть сколько угодно: по файлу на бэкенд в `/opt/rw-backup-restore/s3.d/<имя>.env`. У каждого — свои реквизиты, свой набор категорий (панель / боты / WAL) и **свои сроки хранения**:

```bash
B_UPLOAD_PANEL="true"   B_RETENTION_PANEL_DAYS="10"
B_UPLOAD_CUSTOM="true"  B_RETENTION_CUSTOM_DAYS="30"
B_UPLOAD_WAL="true"     B_BASEBACKUP_KEEP="7"     # WAL чистится по границе этих бэкапов
B_RETENTION_MIN_KEEP="3"
```

Управление — из меню (п. 16) или CLI: `s3-backends`, `s3-add` (интерактивно, с проверкой доступа к бакету), `s3-remove` (удаляет только конфигурацию — данные в бакете не трогаются). Каждая операция бэкапа выгружает архив во **все** включённые бэкенды своей категории с ретраями; недоступность одного бэкенда не мешает остальным. Старые настройки `FULL_EXTERNAL_S3_*` установщик мигрирует в `s3.d/default.env` (с вашего согласия) и продолжает понимать как fallback.

По умолчанию бэкап успешен, если копия ушла хотя бы в одно хранилище (упавший бэкенд даёт лишь `WARN`). Если нужно узнавать о деградации диверсификации сразу — включите **строгий режим** `FULL_S3_STRICT="true"` в `rw-backup-full.env`: тогда задача помечается неуспешной и уходит Telegram-алерт, если копия попала не во все целевые хранилища категории.

## Бэкап и восстановление панели (встроенные)

`panel-backup` создаёт `remnawave_backup_<TS>.tar.gz` в точности в формате оригинала (`dump_<TS>.sql.gz` + `remnawave_dir_<TS>.tar.gz` + `backup_meta.info`), поэтому архивы взаимно совместимы со старым скриптом. Настройки — единые (`PANEL_*` в общем конфиге; если не заданы — читаются из старого `config.env`, ничего не дублируется). Выгрузка: все S3-бэкенды + опционально сам архив в Telegram (`FULL_TG_SEND_PANEL_ARCHIVE=true`, лимит Bot API 50 МБ).

`panel-restore [файл | --from-s3 [--backend NAME]]` — восстановление за 5 шагов, каждый с подробным описанием и подтверждением: остановка панели → каталог в `*.before_restore_*` (не удаляется!) → данные тома БД в резервный volume → чистый старт БД → заливка дампа и полный запуск. Откат возможен на любом шаге.

## Веб-сервис управления парком (песочница)

На множестве серверов вручную следить за бэкапами неудобно — на песочнице поднимается веб-интерфейс (`install.sh --sandbox` предложит установку):

- список всех серверов с живым статусом: компоненты, панель, актуальность бэкапов, WAL-инстансы (спул, базовые бэкапы, свежесть базы и потока WAL), счётчик ошибок;
- **объёмы места по каждому серверу:** занято локально и свободно на диске, а также объём и число объектов в каждом S3-хранилище с пометкой недоступности;
- **вердикт последней проверки** сервера (пройдена/ошибка) и сводная карточка **«Парк»**: сколько серверов online, суммарный объём в S3, суммарно ошибок;
- добавление нового сервера за минуту: разложить публичный SSH-ключ сервиса → «+ Добавить сервер»;
- запуск операций (бэкап панели/ботов, verify, wal-status) в один клик;
- просмотр и правка конфигов (`rw-backup-full.env`, `instances.d/*`, `s3.d/*`) прямо из браузера: секреты маскируются при показе и не затираются при сохранении, перед записью на сервере создаётся резервная копия файла;
- история проверок (парк и стек) и сводка песочницы.

Модель доступа: сервис ходит на серверы **по SSH** и вызывает белый список команд `rw-backup-full` — на прод-серверах не открывается ни одного порта и не ставится никаких агентов. Доступ к интерфейсу — по токену (`/etc/rw-backup-web.env`); сервис слушает `127.0.0.1:8787`, наружу выводите только через reverse-proxy с TLS или VPN.

## Экспорт данных и дашборд Grafana

`rw-metrics-export.timer` (каждые 15 мин) выгружает: свободное/общее место, количество/размер/возраст архивов по категориям, объёмы WAL-хранилищ, объёмы и количество объектов в каждом S3-бэкенде по категориям, доступность бэкендов. Событийные метрики (успехи, длительности, результаты песочницы) пишут сами операции. Всё уходит через textfile collector → vmagent → VictoriaMetrics.

Готовый дашборд — `grafana/dashboard.json` (импорт: Dashboards → Import): статусы бэкапов и песочницы, возрасты, размеры локально и по бэкендам, спул WAL, свободное место, длительности, ошибки. Подробности и рекомендуемые алерты — `grafana/README.md`.

## node_exporter (если ещё не установлен)

`rw-metrics-export.timer` пишет в `/var/lib/node_exporter/textfile_collector` — сами метрики забирает node_exporter, а его — ваш vmagent (push-модель, как и остальной парк). Если node_exporter на сервере ещё нет:

```bash
sudo scripts/host/install-node-exporter.sh
```

Идемпотентен, каждый шаг — с подтверждением; слушает только `127.0.0.1:9100`, наружу не публикуется. Если node_exporter уже стоит из вашего общего Ansible-кита — просто убедитесь, что у него включён `--collector.textfile.directory=/var/lib/node_exporter/textfile_collector`, и этот скрипт не нужен.

## Мониторинг

Сводки в Telegram в **09:00 и 21:00** (`rw-status-digest.timer`) дают краткую картину: события/результаты, занятое и свободное место на дисках (каталог бэкапов и WAL), **объём, занятый бэкапами этого хоста в каждом S3-хранилище** (и пометку недоступности), свежесть бэкапов и число ошибок. На прод-сервере уходят в его Telegram; на песочнице сводка парка дополнительно рассылается по TG-реквизитам всех серверов из манифеста.

Все компоненты пишут метрики в textfile collector (`/var/lib/node_exporter/textfile_collector/`), откуда их забирает vmagent/node_exporter. Ключевые:

| Метрика | Алерт |
|---|---|
| `rw_wal_spool_files` | > 200 — шиппер не справляется / S3 недоступен |
| `rw_wal_ship_failures` | > 0 |
| `rw_basebackup_last_success_timestamp_seconds` | старше 2× интервала |
| `rw_basebackup_last_result` | == 0 |
| `rw_sandbox_pitr_last_ok` | == 0 — **бэкап не восстанавливается** |
| `rw_sandbox_last_run_timestamp_seconds` | старше 2 суток — песочница молчит |

Дополнительно на стороне PostgreSQL стоит алертить `pg_stat_archiver.failed_count` (рост = archive_command падает, `pg_wal` копится).

## Команды (справочник)

```
rw-backup-full                     интерактивное меню (пп. 13–15 — WAL/песочница)
rw-backup-full wal-enable <inst>     включить архивацию (рестарт БД!)
rw-backup-full wal-disable <inst>    выключить и убрать override/таймеры
rw-backup-full basebackup <inst>     полный базовый бэкап сейчас [--no-s3]
rw-backup-full wal-ship <inst>       прогнать шиппер сейчас
rw-backup-full wal-retention <inst>  очистка сейчас [--dry-run]
rw-backup-full wal-timers <inst>     переустановить таймеры (после смены интервалов)
rw-backup-full wal-status            состояние всех инстансов
rw-backup-full pitr-restore <inst>   восстановление (--help — все опции)
rw-backup-full verify                проверка бэкапов в песочнице
rw-backup-full sandbox-timer         применить расписание песочницы из конфига
rw-backup-full s3-backends           список S3-бэкендов
rw-backup-full s3-add / s3-remove    добавить / удалить S3-бэкенд
rw-backup-full s3-test [имя]         диагностика подключений к S3 (шаги + реальные ошибки aws)
rw-backup-full panel-restore         восстановление панели (встроенное)
rw-backup-full status [--json]       статус сервера (JSON — для веб-сервиса)
rw-backup-full metrics-export        выгрузить метрики для Grafana сейчас
```

После изменения расписаний в `instances.d/<inst>.env` выполните `rw-backup-full wal-timers <inst>` — drop-in'ы systemd перегенерируются.

Расписание логических бэкапов v3 настраивается через `rw-backup-full install-timer` (меню «Настройка timer»): интервал в часах или список времён `FULL_TIMER_TIMES` с зоной `FULL_SCHEDULE_TZ`. Расписание песочницы — `SANDBOX_VERIFY_TIMES` / `SANDBOX_VERIFY_INTERVAL_HOURS` в конфиге, применяется командой `rw-backup-full sandbox-timer`.

## Устранение неполадок

**`failed_count` растёт / в логах postgres ошибки archive_command.**
`docker exec <db> ls -la /wal-spool` — каталог должен быть виден и доступен на запись пользователю postgres (uid 999). Если монтирования нет — проверьте, что контейнер пересоздан после `wal-enable` (`docker compose up -d`, не `restart`).

**Спул растёт, S3-выгрузка молчит.**
`journalctl -u rw-wal-ship@<inst>.service -n 50`. Чаще всего — реквизиты S3 или endpoint. Данные не теряются: сегменты копятся локально и уйдут при восстановлении связи. При > 2000 сегментов archive_command начнёт отваливаться намеренно (см. `WAL_SPOOL_MAX_FILES` в `pg-archive-command.sh`) — это защита диска и громкий сигнал.

**`pitr-restore` не находит WAL после базового бэкапа.**
Проверьте, что шиппер работал в момент бэкапа и после (`wal-status`). Восстановиться на момент самого базового бэкапа можно всегда: `--target immediate`.

**Песочница: `бэкап зашифрован, SANDBOX_AGE_IDENTITY не задан`.**
Положите приватный ключ age на песочницу и укажите путь в `rw-backup-full.env`.

**Patroni-кластер.**
`wal-enable` намеренно останавливается: в Patroni параметры `archive_*` задаются через `patronictl edit-config` (DCS), иначе Patroni их перезапишет. Спул/шиппер/basebackup при этом использовать можно — настройте `archive_command` на тот же `archive-command.sh` вручную через DCS.

## Структура проекта

```
install.sh                        установка (роли prod / --sandbox)
scripts/
  rw-backup-full.sh               основной скрипт (v3 + диспетчер v4)
  lib/wal-lib.sh                  общая библиотека WAL-слоя
  wal/
    pg-archive-command.sh         archive_command внутри контейнера (POSIX sh)
    wal-ship.sh                   спул → локальный архив → S3
    basebackup.sh                 pg_basebackup + метаданные + S3
    wal-retention.sh              безопасная очистка (по границе бэкапов)
    enable-archiving.sh           включение/выключение архивации
    wal-timers.sh                 per-instance интервалы таймеров
    pitr-restore.sh               восстановление: полный бэкап / PITR / in-place
  sandbox/verify-backup.sh        автопроверка бэкапов
  panel/panel-backup.sh           встроенный бэкап панели (формат distillium)
  panel/panel-restore.sh          восстановление панели (пошаговые подтверждения)
  metrics/metrics-exporter.sh     выгрузка данных о бэкапах для Grafana
web/app.py + install-web.sh       веб-сервис управления парком (песочница)
grafana/dashboard.json            готовый дашборд Grafana
container/Dockerfile + entrypoint + render-cron   единый образ (все роли)
deploy/host, deploy/sandbox       docker-compose для контейнерного режима
scripts/host/migrate-to-container.sh   systemd -> контейнер (и обратно)
docs/CONTAINER-RU.md              инструкция контейнерного режима
config/
  rw-backup-full.env.example      общий конфиг (v3 + секция v4)
  instances.d/*.example           примеры инстансов (панель, бот)
systemd/                          юниты: ship@, basebackup@, sandbox-verify
```

## Хранилище на диске (прод)

```
/var/lib/rw-wal/<inst>/
  spool/incoming/     сырые сегменты из archive_command (транзит)
  archive/            сжатые [зашифрованные] сегменты — локальная копия
  basebackup/         base_*.tar.zst[.age] + base_*.meta
  state/              маркеры выгрузки в S3, last_success
```

В S3: `<FULL_EXTERNAL_S3_PREFIX>/wal/<hostname>/<inst>/{basebackup,wal}/`. Логические архивы v3 лежат в прежних префиксах `panel/` и `custom-bot/` — форматы и имена v3/оригинала не менялись.

## Совместимость

- Оригинальный `config.env` и его переменные (`BOT_TOKEN`, `CHAT_ID`, `S3_*`) читаются только на чтение.
- Панельный бэкап по умолчанию делает встроенный движок v5; формат `remnawave_backup_<TS>.tar.gz` идентичен оригиналу, архивы взаимно совместимы. Вернуться к оригинальному скрипту: `FULL_PANEL_ENGINE="original"`.
- Из каналов выгрузки оригинала поддержаны Telegram и S3; Google Drive в v5 не перенесён (используйте `FULL_PANEL_ENGINE="original"`, если он необходим).
- Формат архивов ботов `custom_bot_<проект>_<TS>.tar.gz` из v3 не изменён.
- Все v3-команды и меню работают без изменений; v4 — строго аддитивный слой.

## Лицензия

MIT (см. LICENSE).
